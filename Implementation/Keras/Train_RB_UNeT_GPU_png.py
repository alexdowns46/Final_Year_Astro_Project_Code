...
# Define RDB
def RB(inputlayer,channels):
    
	add1=layers.Conv2D(channels,1, padding="same")(inputlayer)
	x = layers.Conv2D(channels,3, padding="same")(add1)
...
if Resume==True:
    kmodel=keras.models.load_model('UNeT_RB_5e_E50.h5',custom_objects={'root_mean_squared_error': root_mean_squared_error,'PSNR': PSNR}) #Load model
    if Recompile==True:
...
callbacks = [
    keras.callbacks.ModelCheckpoint("Unet_RB_Checkpoint.h5",save_best_only=True)
]
...
kmodel.save("UNeT_RB_robust_5e_E55.h5")
