...
netname='UNET_ADD_45EPOCH';
%load(['C:\Users\darth\Desktop\Final Year Project\matlab_files\matlab_files\Networks\' networkfilename]);
netlayers=importKerasLayers('TF_UNET_ADD_E45.h5','OutputLayerType','regression','ImportWeights',true,'ImageInputSize',[512 512 1]);
...
%% Test!
testinglength=length(TestingDataset);
...
writematrix(AveragesDC, [netname '_averages.txt']); %Save results

for j=1:100
    RARR{j}=ResultsDC(j).Rlist;
end
writecell(RARR, [netname '_results.txt']); %Save results
